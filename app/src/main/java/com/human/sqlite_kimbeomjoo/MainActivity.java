...
import com.human.sqlite_kimbeomjoo.DatabaseTables.StudentTable;
...
public class MainActivity extends AppCompatActivity {
    ...
    private SQLiteDatabase mSqLiteDatabase; //안드로이드에서 만들어준//sql템플릿(insert,select...)이 여기 포함
    private RecyclerAdapter mRecyclerAdapter;
    private List mItemList = new ArrayList<StudentVO>();//객체생성,셀렉트 쿼리결과 저장객체
    //입력,수정,삭제 EditText 변수 선언 (->객체로 만들어야 사용가능, 객체 생성 항상실행되는 onCreate메서드에 있음)
    private EditText mEditTextGrade;
    private EditText mEditTextNumber;
    private EditText mEditTextName;
    //현재 RecyclerAdapter에서 선택한 값 확인 변수
    ...
    //가상키보드 제어하는 변수선언
    private InputMethodManager mIinputMethodManager;

    //메인액티비티가 실행되면, 자동으로 실행되는 메서드 onCreate
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //데이터베이스 파일 만들기(아래)
        mSqLiteDatabase = mDatabaseHelper.getWritableDatabase();//싱글톤으로 만들어진 메서드
        // ->싱글톤으로 만들었다는 의미는 인스턴스는 1번만 실행됨.즉 student.db파일이 있으면 생성안됨.
        ...
        //mItemList에 셀렉트 쿼리 결과값이 set되어있어야함.
        //멤버변수로 객체 초기화하기(아래)
        bindObject();
        //List 실행 리사이클러 어댑터 바인딩(아래)
        bindList();//여기서는 공간마련
        //List반영(화면출력 : 입력, 수정, 삭제시 화면 리프레시가 필요하고, 구현하는 메서드가 아래)
        updateList();//여기에서 데이터가 바인딩되어서 RecyclerAdapter가 화면에 재생됩니다.
        //update버튼 클릭이벤트(아래)
        btnUpdate();
        //delete버튼 클릭이벤트(아래)
        btnDelete();
    ...
    //EditText 컴포넌트 입력값 없애기(초기화)
    private void clearComponent() {
        //EditText객체의 값 비우기
        mEditTextGrade.setText("");
        mEditTextNumber.setText("");
        mEditTextName.setText("");
        currentCursorId = -1; //현재 테이블커서ID가 지워졌으니 다시 초기화시킴.
    }
    ...
    private void btnInsert() {
        mButtonInsert.setOnClickListener(v -> {
            ...
            updateList();
            //가상키보드 숨기기
            mIinputMethodManager.hideSoftInputFromWindow(getCurrentFocus().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
        });
        /*
    ...
    private void bindObject() {//onCreate 안에 너무 많아서 따로 모아둠
        ...
        //키보드제어 객체 생성(아래)
        mIinputMethodManager = (InputMethodManager) getSystemService(INPUT_METHOD_SERVICE);
    }
    //화면 리프레시
    private void updateList() {
        mItemList.clear();
        mItemList.addAll(getAllData());
        mRecyclerAdapter.notifyDataSetChanged();//어댑터에 실제값이 들어가면서 화면에 뿌려짐.//안드로이드 전용 클래스이자 메서드
    }

    //셀렉트 쿼리결과를 return합니다.
    private List getAllData() {
        List tableList = new ArrayList<>();//studentTable내용이 담김.
        //쿼리작업에 사용될 필드명 바인딩
        String[] projection = {
        ...
        while(cursor.moveToNext()){//student테이블에 있는 필드값을 하나씩 뽑아서
            ...
            tableList.add(new StudentVO(p_id,p_grade,p_number,p_name));//매개변수가 없는 클래스의 생성자 메서드는 자바가 자동으로 만들어줌.
        }
        return tableList;
    }
